    ...
    private boolean connect(MqttClientParam param) {
        ...
        try {
            ...
            try {
                ...
                // ⭐ 修复: 捕获 MqttAsyncClient 静态初始化时的 MissingResourceException
                // 这是 Unisound SDK 的 bug,MQTT 库尝试加载 logcat_en_CN 资源失败
                // 捕获后让系统稍后重试,避免整个进程崩溃
                this.mConnecting = false;
                LogMgr.e(TAG, "MQTT client init error: " + e.getMessage());
                return false;
            } catch (Exception e) {
            ...
            // ⭐ 修复: 外层也捕获 Error,防止 ExceptionInInitializerError 导致进程崩溃
            this.mConnecting = false;
            LogMgr.e(TAG, "MQTT connection fatal error: " + e2.getMessage());
            return false;
        } catch (Exception e2) {
